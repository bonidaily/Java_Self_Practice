package day_06_methods;

public class daysInMonth {

    public static void main(String[] args) {

        System.out.println(daysInMonth(6));
    }
    public static String daysInMonth(int month){

        String result;
        if(month == 1){
            result = "January has 30 days";
        } else if (month ==2) {
            result = "February has 28 days";
        } else if (month ==3) {
            result = "Mars has 30 days";
        } else if (month == 4) {
            result = "April has 31 days";
        } else if ( month == 5) {
            result = " May has 30 days";
        } else if (month == 6) {
            result = " June has 30 days";
        } else if (month == 7) {
            result = " July has 31 days";
        } else if (month ==8) {
            result = "August has 30 days";
        } else if (month ==9) {
            result = "September has 31 days";
        } else if (month == 10) {
            result = "October has 30 days";
        }else {
            result = "Invalid number";
        }return result;

    }
}
/*
Create a method named daysInMonth that displays the number of days in the month based on the given number.
	If the number is invalid, print "Invalid Number."

				Example:
					daysInMonth(6);

				Output:
					June has 30 days
 */